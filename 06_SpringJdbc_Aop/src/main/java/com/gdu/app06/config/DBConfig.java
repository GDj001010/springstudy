...
@Configuration					// SpringContainer에 bean을 만드는 타입
public class DBConfig {
	
	// DriverManagerDataSource Bean
	@Bean(name="datasource")	// 정확하게 명시는 방법도 있지만 명시 안 해줘도 메소드명이 Bean 이름이 된다.
	...
	// JdbcTemplate이라는 Bean을 만든다 (Connection, PreparedStatement, ResultSet을 이용해서 동작하는 스프링 클래스)
	@Bean
	public JdbcTemplate jdbcTemplate() {
		return new JdbcTemplate(dataSource());	//DriverManagerDataSource Bean을 JdbcTemplate(Bean) 생성자에 주입
	}
