...
public class SecurityUtil {
  ...
  // SHA-256 암호화하기
  /*
     1. 입력 값을 256비트(32 바이트) 암호화 처리하는 해시 알고리즘이다.
  ...
  public String getSha256(String str) {
    ...
    try {
      ...
      messageDigest.update(str.getBytes()); // 전달된 문자열(비밀번호를)을 암호화(byte 배열로) 한다.
    } catch(Exception e) {
      e.printStackTrace();
    }
    ...
    for(int i = 0; i < b.length; i++) {
      sb.append(String.format("%2X", b[i]));  // %X : 16진수를 의미한다, %2X : 2자리의 16진수를 의미 (X가 대문자면 16진수 알파벳이 대문자로 나타난다.)
    }
    return sb.toString();
  }
