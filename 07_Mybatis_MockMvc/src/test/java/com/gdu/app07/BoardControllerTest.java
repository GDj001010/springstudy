...
// ContextConfiguration   Bean은 어떻게 만들었나, Bean 관리를 어떻게 하나.
...
// WebApplicationContext를 사용하고 싶으면 명시해준다.
@WebAppConfiguration 
public class BoardControllerTest {
	...
	*/
	...
	// @Brfore
	// 1. 모든 @Test 수행 이전에 실행한다.
	// 2. MockMvc mockMvc 객체를 @Before에서 build한다.
	@Before
	public void setUp() throws Exception {
		mockMvc = MockMvcBuilders		// mockMvc  초기화 
	...
	public void a3상세조회테스트() throws Exception{
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
							...
							.param("boardNo", "1"))		// 파라미터
								.andReturn()			// 조회결과
								.getModelAndView()		// Model에 저장된 조회 결과를 가져오기 위해서 ModelAndView
	...
	public void a4목록테스트() throws Exception{
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
							.get("/board/list.do"))		// @GetMapping("board/detail.do")
								.andReturn()			// 목록조회결과
								.getModelAndView()		// Model에 저장된 조회 결과를 가져오기 위해서 ModelAndView를
								.getModelMap()			// ModelAndView에서 Model을 가져옴
									.toString());		
