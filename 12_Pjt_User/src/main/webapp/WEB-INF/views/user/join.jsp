...
<script>
  ...
  function fnCheckId(){
	  $('#id').on('keyup', function(){
		  ...
		  // 정규식 (5~40자, 소문자+숫자+하이픈(-)+밑줄(_) 사용가능, 첫 글자는 소문자+숫자 사용 가능)
		  let regId = /^[a-z0-9][a-z0-9-_]{4,39}$/;
		  
		  // 정규식 검사
		  verifyId = regId.test(id);
		  if(verifyId == false){
			  $('#msgId').text('(5~40자, 소문자+숫자+하이픈(-)+밑줄(_) 사용가능, 첫 글자는 소문자+숫자 사용 가능)');
  ...
  function fnCheckPw(){
	  ...
	  $('#pw').on('keyup', function(){
		  ...
		  // 정규식 (4~20자, 소문자, 대문자, 숫자, 특수문자 가능, 3개 이상 조합)
		  ...
		  let validCount = /[a-z]/.test(pw)         // 소문자를 가지고 있으면 true(1), 없으면 false(0)
		                 ...
		                 + /[^a-zA-Z0-9]/.test(pw)  // 특수문자를 가지고 있으면 true(1), 없으면 false(0)  [] < 대괄호 안에 ^가 들어간다면 제외한단 뜻이 된다.
		  verifyPw = (pwLength >= 4) && (pwLength <= 20) && (validCount >= 3);
		  if(verifyPw){
			  $('#msgPw').text('사용 가능한 비밀번호입니다.');
		  } else {
			  $('#msgPw').text('4~20자, 소문자, 대문자, 숫자, 특수문자 가능, 3개 이상 조합');
		  }
