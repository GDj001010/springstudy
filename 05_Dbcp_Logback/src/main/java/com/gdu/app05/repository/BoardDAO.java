...
import com.gdu.app05.domain.BoardDTO;
				...
				// Spring Container에 저장, DAO가 사용하는 @Componen이다, Spring Container에 Bean이 등록될 때 Singleton으로 등록되기 때문에 별도의 Singleton Pattern 코드를 작성할 필요가 없다.
public class BoardDAO {
	...
	// jdbc에선 connection을 직접 만들어줬지만 dbcp에선(context.xml에 작성한 <Resource> 태그가 있기 때문에) 아니다.
	// BoardDAO 생성자 (webapp/MATA-INF/context.xml에 작성한 <Resource> 태그 읽기
	public BoardDAO() {
		// JNDI 방식 : <Resource> 태그의 name 속성으로 Resource를 읽어 들이는 방식
		// lookup 은 Object 타입으로 반환한다
		try {
	...
	public List<BoardDTO> selectBoardList() {
		...
		try {
			// dataSource엔 8개의 Connection이 있다.
	...
	public BoardDTO selectBoardByNo(int board_no) {
		...
		try {
			...
			sql = "SELECT BOARD_NO, TITLE, CONTENT, WRITER, CREATED_AT, MODIFIED_AT FROM BOARD WHERE BOARD_NO = ?";	// 변수값은 무조건 "?" 로 넣어준다, 보안문제가 있기 때문
			ps = con.prepareStatement(sql);
