...
// @EnableAspectJAutoProxy	// DBConfig 에 이미 애너테이션이 들어가있기 때문에 넣어줄 필요가 없다.
public class ParameterCheckAOP {
	...
	@After("setPointCut()")	// Around 제외 모든 것들은 JoinPint 사용
	public void paramLogging(JoinPoint joinPoint) throws Throwable {
		...
		// 모든 파라미터가 Map의 Key로 변환된다. Map의 Key로 변환된다. Map의 Key는 반복문으로 모두 순회할 수 있다.
