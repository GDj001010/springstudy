...
<script>
	...
	var totalPage = 0;	// 전체 페이지 개수(서비스에서 받아온다.)
		...
		$.ajax({
			...
			success: function(resData){		// resData = {"employees": [{}, {}, {}, ...], "totalPage": 12}
				...
				$.each(resData.employees, function(i, employee){
					...
					str += '</div>';		// script에선 닫은 태그 작성 생략이 가능하다.
	...
	// 스크롤 이벤트
	$(window).on('scroll', function(){	// java script의 최상위 객체 = window		jquery 객체로 바꾼 이유는 호출하는 메소드가 jquery 메소드이기 떄문
		
		if(timerId){
			clearTimeout(timerId);		// setTimeout()이 동작했다면 (목록을 가져왔다면) setTimeout()의 재동작을 취소한다. 동일 목록 가져오는 것을 방지
		}
		
		// timerId = setTimeout(function(){}, 200);  // 200밀리초(0.2초) 후에 function()을 수행한다.
		timerId = setTimeout(function(){	// 500밀리초 (0.5초) 후에 function()을 수행한다
...
<style>
	...
	.employees{
		width: 1000px;
		margin: 0 auto;		/* 중간 정렬 */
		display: flex;		/* 자식들 좌, 우 배치 */
		flex-wrap: wrap;	/* 기본값은 nowrap이기 때문에 명시를 안 해주면 한 줄에 9개를 때려박는다. */
	}
	.employee{
		width: 300px;
		height: 300px;
		...
		margin: 10px;
		/* 박스 안 내용이 화면 중앙(가로, 세로 가운데)에 배치하는 방법 */
